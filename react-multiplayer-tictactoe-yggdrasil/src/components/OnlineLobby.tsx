...
export const OnlineLobby: React.FC<OnlineLobbyProps> = ({ username, onBack, onJoinRoom, onCreateRoom }) => {
  ...
  const handleCreate = async () => {
    ...
    setError("");
    try {
      console.log("=== OnlineLobby.handleCreate calling onCreateRoom ===");
      await onCreateRoom();
      console.log("=== OnlineLobby.handleCreate onCreateRoom success ===");
      setIsCreating(false);
    } catch (err: any) {
      console.error("=== OnlineLobby.handleCreate error:", err);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <header className="absolute top-6 left-6 z-10">
        <button onClick={onBack} className="flex items-center space-x-2 px-3.5 py-2 bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded-xl text-xs font-medium text-slate-300 transition-colors cursor-pointer">
          <ArrowLeft className="w-4 h-4" />
          <span>Kembali</span>
        </button>
      </header>

      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-8 shadow-2xl relative z-10">
        ...
        <div className="space-y-6">
          ...
          <form onSubmit={handleJoin} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Kode Room Unik (6 Karakter)</label>
